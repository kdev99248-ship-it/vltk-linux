...
function wlls_create(szParam)
	...
	if(not FALSE(n_lid)) then
		wlls_say(rolename, "ChiÕn ®éi cña [<color=yellow>"..leaguename.."<color>] ®· ®­îc thµnh lËp.")
		return 0
	end
	...
	if(wlls_relay_createleague(leaguename) == 1) then
		...
		if(wlls_relay_addmember(leaguename, rolename) == 1) then
			...
			wlls_say(rolename, "ChiÕn ®éi cña [<color=yellow>"..leaguename.."<color>] ®· ®­îc thµnh lËp.", nil, 1)
			-- zzMsg2Player = "ChiÕn ®éi cña [<color=yellow>"..leaguename.."<color>] ®· ®­îc thµnh lËp."
...
function wlls_leave(rolename)
	...
	local str = "ChiÕn ®éi cña [<color=yellow>"..leaguename.."<color>]."
	local removelg = 0
