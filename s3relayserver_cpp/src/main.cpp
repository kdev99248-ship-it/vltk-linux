...
Config load_config(const std::string& root) {
    ...
    if (relay.empty()) {
        ...
        if (local.count("relaxed_verify")) cfg.relay_store.relaxed_verify = parse_bool(local["relaxed_verify"]);
    }

    // Read the MySQL connection settings from db.ini (preferred); mssql.ini is
    // accepted as a legacy filename. Later files/sections override earlier ones.
    auto apply_db = [&](const std::string& path) {
        auto db = read_ini(path);
        auto read_db = [&](const std::string& key, std::string& target) {
            for (const char* section : {"", "db", "DB", "mysql", "MySQL", "MYSQL", "mssql", "MSSQL"}) {
                if (db.count(section) && db[section].count(key)) target = db[section][key];
            }
        };
    ...
    apply_db(root + "/mssql.ini");
    apply_db(root + "/db.ini");
    return cfg;
}
...
bool handle_windows_verify(Client& client,
    ...
    if (version != 1 || type != 3) {
        if (!state.relay_store->native_protocol()) {
    ...
    if (!verify.ok) {
        const bool windows_backend = state.relay_store->native_protocol();
        if (!verify.found) {
            if (!windows_backend) {
                queue_canonical_verify_ack(client, body, table, 4);
                return true;
...
bool handle_windows_logout(Client& client,
    ...
    if (!require_windows_verify(client, body, table, "c2s_accountlogout",
                                state.relay_store->native_protocol())) return true;
    if (state.relay_store->native_protocol()) {
